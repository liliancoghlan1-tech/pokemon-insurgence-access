module PokeAccess
  ...
  module Locator
    ...
    TCAT_KEYS = { :all => :tcat_all, :people => :tcat_people, :objects => :tcat_objects,
                  ...
                  :marks => :tcat_marks, :roamers => :tcat_roamers, :unseen => :tcat_unseen,
                  :story => :tcat_story }

    # The spoken name of a target category.
    #
    # A registered category brings its own :label, because TCAT_KEYS is a closed table and anything missing
    # from it fell through to "everything" -- so a profile's category announced itself as the all-targets
    # one and was indistinguishable from it. That is exactly how the Tesseract category looked absent when
    # it was in fact being offered and cycled past.
    def self.cat_name(cat)
      o = EXTRA_CATEGORIES[cat]
      return PokeAccess::I18n.t(o[:label]) if o && o[:label]
      PokeAccess::I18n.t(TCAT_KEYS[cat] || :tcat_all)
    end

    # A target on ANOTHER map, kept alive across the door that took her off its map. Selecting the gym
    # leader and then crawling into a tree used to drop the target on the floor; now it becomes one of
    # these and the guide keeps routing to it, one door at a time.
    RemoteTarget = Struct.new(:map_id, :x, :y, :name)

    # True if a target is on a map other than this one.
    def self.remote_target?(t); t.is_a?(RemoteTarget); end

    # The map a target is on (this one, unless it says otherwise).
    def self.target_map(t)
      remote_target?(t) ? t.map_id : ($game_map.map_id rescue nil)
    end
    ...
    def self.in_category?(ev, cat)
      ...
      case cat
      when :exits  then transfer_event?(ev) || conditional_transfer?(ev) || mechanic_exit?(ev)
      when :signs  then sign_event?(ev)
      when :extras then !named && examinable?(ev) && !sign_event?(ev)
      when :lens   then lens_tile?(ev)
      when :roamers then roamer?(ev)
      when :unseen then PokeAccess::Seen.candidate?(ev) &&
                        !PokeAccess::Seen.seen?(($game_map.map_id rescue nil), ev.id)
      when :all    then named || transfer_event?(ev) || conditional_transfer?(ev) ||
                          examinable?(ev) || lens_tile?(ev) || roamer?(ev) || mechanic_exit?(ev)
    ...
    # Scanner categories a GAME profile contributes, key => {:available, :targets, :place}.
    #
    # Story is the shape being generalised: a category that is not an event scan at all, whose targets may
    # live on OTHER maps, ranked by how many doors away they are, and named by place rather than by
    # coordinates. Insurgence's Tesseract rifts want exactly that and are the game's own knowledge, so they
    # register rather than being written into the core.
    #
    #   :available -> is there anything to list right now
    #   :targets   -> the target list (SurfaceTarget here, a RemoteTarget elsewhere)
    #   :place     -> optional, how to speak a remote one of these (see announce_selected)
    EXTRA_CATEGORIES = {}

    def self.register_category(key, opts); EXTRA_CATEGORIES[key] = opts; end

    # The registered categories with something in them. A provider that raises simply does not appear --
    # a broken profile must never be able to empty the scanner.
    def self.extra_category_keys
      return [] if EXTRA_CATEGORIES.empty?
      EXTRA_CATEGORIES.keys.select { |k| (EXTRA_CATEGORIES[k][:available].call rescue false) ? true : false }
    rescue StandardError
      []
    end

    # A registered category's spoken name for a remote target, or nil.
    def self.extra_place_phrase(t)
      return nil if EXTRA_CATEGORIES.empty?
      EXTRA_CATEGORIES.each_value do |o|
        p = o[:place]
        next unless p
        r = (p.call(t) rescue nil)
        return r if r
      end
      nil
    rescue StandardError
    ...
    def self.active_categories
      ...
      base += [:unseen] if unseen_here?
      base += [:roamers] if roamers_here?
      # Story steps, once the index is built and something is waiting (see Story).
      base += [:story] if (PokeAccess::Story.ready? && PokeAccess::Story.any? rescue false)
      base += [:marks] if marks_here?
      base += extra_category_keys
      base
    end

    # True if anything on this map has not been looked at yet, gating the :unseen category so it never
    # shows up empty -- which is also what makes its absence meaningful: no category, nothing left here.
    def self.unseen_here?
      !(PokeAccess::Seen.unseen_on_map.empty? rescue true)
    end

    # True if the current map holds a hidden roamer, gating the :roamers category. Unlike every other
    # category this one can EMPTY ITSELF mid-map: catching the thing flips its event to a page with no
    # autonomous movement, which is exactly when the category should stop being offered.
    def self.roamers_here?
      !!(PokeAccess::Roamers.any? rescue false)
    end

    # True if an event is a hidden roamer (see Roamers), asked here so in_category? reads like its
    # neighbours; the definition itself lives with the rest of the roamer handling.
    def self.roamer?(ev)
      PokeAccess::Roamers.roamer?(ev)
    rescue StandardError
      false
    end
    ...
    def self.rebuild_targets
      ...
      cat = cats[@cat]
      synthetic = (cat == :surfaces || cat == :puzzles || cat == :marks || cat == :story ||
                   EXTRA_CATEGORIES.has_key?(cat))
      if EXTRA_CATEGORIES.has_key?(cat)
        @targets = (EXTRA_CATEGORIES[cat][:targets].call rescue [])
      elsif cat == :story
        @targets = (PokeAccess::Story.targets rescue [])
      elsif cat == :surfaces
        ...
        @targets.concat(connection_targets.map { |ct| aim_connection(ct) }) if cat == :exits || cat == :all
      end
      @targets = @targets.sort_by { |ev| (ev.x - px).abs + (ev.y - py).abs }
      @targets = reachable_first(@targets) unless synthetic
      if !synthetic && (PokeAccess::Config.hide_noninteractive rescue false)
        @targets = @targets.select { |ev| ev.is_a?(SurfaceTarget) || interactable?(ev) || roamer?(ev) }
      end
    ...
    #
    # The list with what she can actually get to first -- on foot, past a field move, or by surfing -- and the
    # rest after it, each group still nearest first. A cave map in Insurgence is a dozen separate rooms, and
    # the four things in HER room were buried among twenty "no reachable route"s from rooms she is not in.
    # Nothing is hidden (that is the hide-unreachable option's job); only the order changes.
    def self.reachable_first(list)
      return list if list.length <= 1
      return list unless (PokeAccess::Pathfinder.reachable_set_complete? rescue false)
      ss = (($PokemonGlobal.surfing rescue false) ? nil : (PokeAccess::Pathfinder.surf_reachable_set(false) rescue nil))
      pk = PokeAccess::Pathfinder
      ranked = list.each_with_index.map do |ev, i|
        r = if reachable?(ev) then 0
            elsif ss && [[0, 0], [0, 1], [0, -1], [1, 0], [-1, 0]].any? { |a, b| ss[pk.pkey(ev.x + a, ev.y + b)] } then 1
            else 2
            end
        [r, i, ev]
      end
      ranked.sort_by { |r, i, _e| [r, i] }.map { |_r, _i, e| e }
    rescue StandardError
      list
    end
    ...
    def self.reachable?(ev)
      ...
      return true if [[-1, 0], [1, 0], [0, -1], [0, 1]].any? do |dx, dy|
        ($game_map.counter?(tx + dx, ty + dy) rescue false) && !!s[pf.pkey(tx + 2 * dx, ty + 2 * dy)]
      end
      # Last, and only for the handful the flood really could not reach: a target behind a cuttable tree
      # IS reachable, it just costs a move first, so the hide-unreachable filter must not delete it. Asked
      # here rather than folded into the flood because it is one A* per target and this is the one branch
      # where that cost is already unavoidable.
      return true if !(PokeAccess::Gates.empty? rescue true) &&
                     (PokeAccess::Pathfinder.find_path_gated(tx, ty, false) rescue nil)
      false
    rescue StandardError
      true
    end
    ...
    def self.target_valid?
      ...
      return true if remote_target?(@target)
    ...
    def self.ensure_target
      unless target_valid?
        ...
        @ti = 0 if @ti < 0
        @target = @targets[@ti]
        @target_map = ($game_map.map_id rescue nil)
      end
    end
    ...
    def self.step_phrase(target)
      if remote_target?(target)
        return ", " + PokeAccess::I18n.t(no_route_key) unless (w = warp_phrase(target))
        return w
      end
      path = (target_path(target) rescue nil)
      if path.nil?
        # Nothing walkable -- but "no reachable route" is the answer that tells a blind player the place
        # does not exist, so it is the LAST thing said, not the first. A field-move obstacle in the way is
        # a distance plus a name, not a wall.
        gp = (PokeAccess::Pathfinder.find_path_gated(target.x, target.y) rescue nil)
        if gp
          note = (PokeAccess::Gates.needs_phrase(gp[1]) rescue nil)
          steps = PokeAccess::I18n.t(:loc_steps, :n => gp[0].length)
          return ", " + steps + (note ? ", " + note : "")
        end
        # Doors come after field moves and before surfing: clearing one obstacle in front of her beats
        # walking a circuit of the building, and a circuit beats nothing at all.
        w = warp_phrase(target)
        return w if w
        return ", " + PokeAccess::I18n.t(:loc_surf_route) if (PokeAccess::Pathfinder.surf_launch(target.x, target.y) rescue nil)
        sg = surf_then_phrase(target)
        return ", " + sg if sg
        return ", " + PokeAccess::I18n.t(no_route_key)
      end
    ...
    # The walking route to a target as the guide will walk it: onto the tile for a dive spot or a place to
    # surface (pressed ON), beside it for everything else.
    def self.target_path(t)
      dive_target?(t) ? PokeAccess::Pathfinder.path_onto(t.x, t.y) : PokeAccess::Pathfinder.find_path(t.x, t.y)
    end

    # Which "there is no route" to say. A route search that ran out of nodes or seconds has not proved
    # anything, and the flood behind it may itself have been cut short: in that case the honest answer is
    # that the mod could not work it out, not that the place cannot be reached.
    def self.no_route_key
      full = (PokeAccess::Pathfinder.reachable_set_complete? rescue false)
      searched = (PokeAccess::WarpNet.searched_fully? rescue false)
      (full && searched) ? :loc_no_route : :loc_no_route_unsure
    end

    # The steps-to-the-next-door phrase for a target only reachable through one, or nil.
    def self.warp_phrase(target)
      r = (PokeAccess::Pathfinder.find_path_warp(target_map(target), target.x, target.y) rescue nil)
      return nil if r.nil?
      ", " + PokeAccess::I18n.t(:loc_steps, :n => r[0].length) + ", " + doors_text(r)
    rescue StandardError
      nil
    end

    # "3 ways through", or "3 ways through, then across the water" when the doors only lead to a shore.
    def self.doors_text(r)
      s = PokeAccess::I18n.t(r[1] == 1 ? :loc_doors_one : :loc_doors, :n => r[1])
      s = "#{s}, #{PokeAccess::I18n.t(:loc_then_surf)}" if r[2] == :surf
      # The first crossing is a dive rather than a doorway.
      s = "#{s}, #{PokeAccess::I18n.t(r[5] == :dive ? :loc_then_dive : :loc_then_surface)}" if r[5]
      # The first door is itself across water: that comes first.
      s = PokeAccess::I18n.t(:loc_surf_then, :needs => s) if r[4]
      s
    end
    ...
    def self.announce_selected(withname)
      ...
      if remote_target?(@target)
        nm = (PokeAccess::Story.place_phrase(@target) rescue nil) ||
             extra_place_phrase(@target) ||
             PokeAccess::I18n.t(:loc_on_map, :name => @target.name.to_s,
                                :map => (map_name(@target.map_id) || "").to_s)
        return PokeAccess.speak("#{nm}#{step_phrase(@target)}", true)
      end
    ...
    def self.announce_route
      ensure_target
      return PokeAccess.speak(PokeAccess::I18n.t(:loc_nothing_selected), true) if @target.nil?
      PokeAccess.speak(PokeAccess::I18n.t(:loc_route, :steps => route_text(@target)), true)
    end

    # The spoken directions to a target, by the SAME ladder the guide walks: a plain route, else one leg to
    # a field-move obstacle, else one leg to the next door, else the shore.
    #
    # The route key used to ask find_path and nothing else. So a target the cane could reach perfectly well
    # answered "no reachable route" the moment she asked HOW to get there -- which is exactly the Rezzai
    # Desert report: the exits list said "exit to Roggan Town, 12 steps away, 2 ways through", and pressing
    # for directions said there was no route at all. Every other reader had learned about doors; this one
    # had not.
    def self.route_text(t)
      unless remote_target?(t)
        p = (target_path(t) rescue nil)
        return PokeAccess::Pathfinder.path_to_text(p) if p
        gp = (PokeAccess::Pathfinder.find_path_gated(t.x, t.y) rescue nil)
        if gp
          note = (PokeAccess::Gates.needs_phrase(gp[1]) rescue nil)
          txt = PokeAccess::Pathfinder.path_to_text(gp[0])
          return note ? "#{txt}, #{note}" : txt
        end
      end
      w = (PokeAccess::Pathfinder.find_path_warp(target_map(t), t.x, t.y) rescue nil)
      if w
        txt = PokeAccess::Pathfinder.path_to_text(w[0])
        return txt + ", " + doors_text(w)
      end
      unless remote_target?(t)
        return PokeAccess::I18n.t(:loc_surf_route) if (PokeAccess::Pathfinder.surf_launch(t.x, t.y) rescue nil)
        sg = surf_then_phrase(t)
        return sg if sg
      end
      PokeAccess::I18n.t(no_route_key)
    rescue StandardError
      PokeAccess::I18n.t(no_route_key)
    end

    # "across the water, then needs Rock Climb, and you don't have the Hiking Boots", or nil. For a target
    # past the water AND behind an obstacle on the far side -- Route 17 out of Helios City sits on a ledge
    # of rock across the bay. Plain "no reachable route" there told her an exit she could see on the map
    # did not exist.
    def self.surf_then_phrase(t)
      sg = (PokeAccess::Pathfinder.surf_launch_gated(t.x, t.y) rescue nil)
      return nil if sg.nil?
      note = (PokeAccess::Gates.needs_phrase(sg[1]) rescue nil)
      return nil if note.nil?
      PokeAccess::I18n.t(:loc_surf_then, :needs => note)
    rescue StandardError
      nil
    end
    ...
    def self.announce_map_change
      ...
      @map_changed_at = (PokeAccess.clock rescue nil)
      forget_route_state
      @targets = []; @target = nil; @ti = 0
      (rebuild_targets rescue nil)
      restore_carried(mid)
      if @through_target_door
        @through_target_door = false
        stop_guides(:loc_arrived) if @guide || @steps
      end
      nm = (map_name(mid) rescue nil)
      nm = "#{nm}#{unseen_suffix}" if nm && !nm.to_s.strip.empty?
      PokeAccess.speak(nm, false) if nm && !nm.to_s.strip.empty?
    end

    # Puts a target carried through a door back in hand. On its OWN map the real event is in the list
    # again, so the carry is simply dropped; anywhere else it becomes the selection, and the guide keeps
    # working toward it one door at a time. Silent: she asked for this target once and has not changed it.
    def self.restore_carried(mid)
      c = @carried
      return if c.nil?
      unless c.map_id == mid
        @target = c
      ...
      # Back on the target's OWN map: hand the selection to the real thing standing there, so the guide
      # carries straight on. Dropping the carry and stopping here was the last door of the journey
      # announcing "target lost" -- the one moment she is nearly on top of it.
      @carried = nil
      ev = ($game_map.events.values.detect { |e| e.x == c.x && e.y == c.y } rescue nil)
      # A story step arrives as a story step: handing it to the real event would read out who is standing there.
      ev = nil if c.is_a?(PokeAccess::Story::StoryTarget)
      @target = ev || SurfaceTarget.new(c.x, c.y, c.name.to_s, :carried)
      @target_map = mid
      idx = (@targets.index(@target) rescue nil)
      @ti = idx if idx
    rescue StandardError
      nil
    end

    # Forgets a carried target (a new selection replaces it).
    def self.drop_carried; @carried = nil; end

    # ", 12 not looked at" appended to the map name on arrival, when the player asked for it. Off by
    # default: it is a useful thing to hear walking into a new town and a tedious one walking through a
    # corridor for the fifth time.
    def self.unseen_suffix
      return "" unless (PokeAccess::Config.unseen_on_arrival rescue false)
      n = (PokeAccess::Seen.count_here rescue 0)
      return "" if n <= 0
      ", " + PokeAccess::I18n.t(:loc_unseen_count, :n => n)
    rescue StandardError
      ""
    end
    ...
    def self.clear_targets
      # Carry the selection through the door as a remote target, so "take me to the gym leader" survives
      # the tree she just crawled into. Only a real, placed target is worth carrying.
      # The map the target was SELECTED on, never the current one: this runs from the cache reset that a
      # map change triggers, by which point $game_map is already the map she just walked INTO. Recording
      # that made the carry look like it was already here, so it was dropped on arrival and the guide
      # announced "target lost" the moment she crawled into the first tree.
      # Except the doorway she was heading for, walked through: that was the destination, not a place to
      # lead her back to. Carried, the guide turned round on the far side and said "door to the north".
      dm = (@target && !remote_target?(@target)) ? (transfer_command_dest(@target) rescue nil) : nil
      # A dive spot or a place to surface is the same kind of doorway: pressing on it is the journey's end.
      dm = (PokeAccess::Dive.lands_on(@target.key, @target_map) rescue nil) if dive_target?(@target) && @target_map
      if dm && dm == ($game_map.map_id rescue nil) && dm != @target_map
        @through_target_door = true
        @carried = nil
      elsif @target && !remote_target?(@target)
        nm = (target_name(@target) rescue nil)
        tm = @target_map || ($game_map.map_id rescue nil)
        @carried = RemoteTarget.new(tm, @target.x, @target.y, nm) if nm && tm
      elsif remote_target?(@target)
        # A target chosen on another map to begin with (a story step) is carried as it is.
        @carried = @target
      end
      @targets = []; @target = nil; @ti = 0
    ...
    def self.refresh_on_event_end
      ...
      if @interp_running && !run
        ...
        (PokeAccess::Roamers.forget rescue nil)
    ...
    # Seconds the guides wait after arriving on a map before asking for a route.
    SETTLE_SECONDS = 0.5

    # True while a transfer is still in progress or has only just finished. A route asked for on the first
    # frame of a new map was asked about a player the engine had not finished placing, came back "no
    # reachable route", was spoken, and was latched until she took a step: walking through the museum door
    # on a route that plainly continued, she heard that there was no way on.
    def self.map_settling?
      return true if (($game_temp.player_transferring rescue false) || ($game_temp.transition_processing rescue false))
      # An event already running -- the door she has just stepped onto, before it has moved her: a route asked
      # for from the door tile itself came out as a stray "1 right" in the instant before the transfer.
      return true if ($game_system.map_interpreter.running? rescue false)
      # A scripted move still carrying her (the step off a door tile, with walls switched off): wait it out.
      return true if ($game_player.move_route_forcing rescue false)
      # Standing on a doorway she walked into the right way: the transfer starts next frame. A route planned
      # from the doorway tile was thrown away a moment later and cost four seconds of frozen game.
      return true if (PokeAccess::Pathfinder.exit_tile?($game_player.x, $game_player.y, $game_player.direction) rescue false)
      @map_changed_at && (PokeAccess.clock - @map_changed_at) < SETTLE_SECONDS
    rescue StandardError
      false
    end
    ...
    def self.map_poll
      ...
      PokeAccess::Roamers.tick rescue nil
      PokeAccess::Assist.sync_expshare rescue nil
      unless map_settling?
        guide_tick
        steps_tick
      end
