    ...
    def self.global_poll
      ...
      if key(:info)
        if shift_down?
          ...
          # Plain info on a puzzle that has a walkthrough says the NEXT INSTRUCTION and nothing else.
          # The full readout -- beam traces, every mirror, the assist line -- is a page of speech to sit
          # through when what she wants is the one line telling her what to do, so it moves to Ctrl.
          # A puzzle with no walkthrough is unaffected: :brief falls straight through to the full read.
          PokeAccess::Puzzles.read(ctrl_down? ? :full : :brief)
        elsif (mg = (PokeAccess::Minigames.info_text rescue nil))
          # A minigame that can answer "what does the board look like" takes the info key while it is up.
          PokeAccess.speak(mg, true)
        else
