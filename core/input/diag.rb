module PokeAccess
  ...
  module Keys
    ...
    DIAG_ALL = [:diag_perf, :diag_focus, :diag_map, :diag_locator, :diag_pathfinder, :diag_puzzle,
                :diag_links, :diag_surface, :diag_audio3d, :diag_scene, :diag_runtime, :diag_polls]
    ...
    DIAG_SECTIONS = {
      ...
      :map    => [:diag_map, :diag_pathfinder, :diag_puzzle, :diag_links, :diag_surface],
    ...
    def self.diag_pathfinder(o)
      ...
      # The mechanism edges found on this map. A cave whose halves join through a staircase reads as two
      # unreachable halves if these come back empty, so this is the first thing to look at when a target
      # that is plainly walkable reports no route.
      st = PokeAccess::Pathfinder::PKEY_STRIDE
      fm = dv { pf.slide_index }
      wp = dv { pf.warp_index }
      if fm.is_a?(Hash) && wp.is_a?(Hash)
        bump = fm.keys.select { |k| pf.landing(k / st, k % st).nil? }
        multi = fm.values.select { |v| v.size > 1 }
        o.push("edges: forced_move=#{fm.size} (bump/staircase=#{bump.size} multi_branch=#{multi.size}) internal_warp=#{wp.size}")
        o.push("  forced=#{cut(fm.keys.map { |k| "#{k / st},#{k % st}#{fm[k].keys.inspect}" }.inspect, 220)}") unless fm.empty?
        o.push("  warps=#{cut(wp.keys.map { |k| "#{k / st},#{k % st}->#{wp[k].inspect}" }.inspect, 220)}") unless wp.empty?
      end
    ...
    # Profile-contributed crossings (Insurgence's Tesseract) as the ROUTER sees them, which is the only
    # place the truth lives: a link row is silently dropped if the destination map's region cannot be
    # modelled, so "it is registered" and "it is routable" are different claims and only this can tell them
    # apart. Reports, per row: where it crosses to, and how big a region the router thinks it lands in.
    def self.diag_links(o)
      wn = PokeAccess::WarpNet
      mid = dv { $game_map.map_id }
      rows = dv { wn.extra_links(mid) }
      unless rows.is_a?(Array)
        return o.push("links: extra_links unavailable (#{rows.inspect})")
      end
      if rows.empty?
        return o.push("links: none offered on map #{mid} (sources=#{dv { wn::LINK_SOURCES.length }})")
      end
      here = dv { wn.region(mid, $game_player.x, $game_player.y) }
      o.push("links: #{rows.length} on map #{mid}, sources=#{dv { wn::LINK_SOURCES.length }}, her region=#{dv { here ? here.length : nil }}")
      # Grouped by the size of the region each lands in: one line per distinct destination region is what
      # answers "does any of these actually reach somewhere new", which listing the first six did not.
      groups = {}
      rows.each do |wx, wy, dm, dx, dy, face, kind|
        r = dv { wn.region(dm, dx, dy) }
        n = r.is_a?(Hash) ? r.length : nil
        k = [dm, n]
        (groups[k] ||= []).push("#{wx},#{wy}")
      end
      groups.each do |k, tiles|
        dm, n = k
        o.push("  -> map #{dm} landing_region=#{n ? n : 'NIL (edge dropped)'} from #{tiles.length} tile(s): #{cut(tiles.join(' '), 120)}")
      end
    rescue StandardError => e
      o.push("links: ERR #{e.class}: #{e.message}")
    end
