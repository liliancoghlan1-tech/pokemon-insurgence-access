module PokeAccess
  ...
  module Puzzles
    ...
    def self.active?
      ...
      # Outstanding walkthrough steps make a puzzle active whatever its kind says. Without this a puzzle
      # that exists ONLY to carry a walkthrough (Crystal Caves' boulder room) is inactive, the info key
      # never reaches Puzzles.read, and the one key meant to repeat the instruction says nothing.
      return true unless (PokeAccess::Walkthrough.steps(d) || []).empty?
      case kind(d)
      when :state  then state_active?(d)
      when :facing then !facing_statues(d).empty?
      when :push   then PokeAccess::Push.active?(d)
      when :laser  then PokeAccess::Laser.active?(d)
      when :walkthrough then false
      else !!(d[:active].call)
      end
    ...
    def self.reset_state
      ...
      @controls = nil; @controls_map = nil
      @statues = nil; @statues_map = nil; @face_last = nil
      PokeAccess::Push.reset rescue nil
    end
    ...
    def self.tick
      ...
      when :push   then PokeAccess::Push.tick(d)
      when :laser  then PokeAccess::Laser.tick(d)
      when :walkthrough then nil
      else grid_tick(d)
      end
      obstacle_tick(d) if d[:obstacles]
      # After the kind's own frame, so a step that just completed is announced against fresh state.
      PokeAccess::Walkthrough.tick(d)
    rescue StandardError
      nil
    end
    ...
    # The info-key readout; dispatches to the puzzle kind. The walkthrough line, when the puzzle has one,
    # is spoken first: it is the answer to "what do I do", and she should not have to sit through a board
    # readout to reach it.
    def self.read(mode = :full)
      d = current
      return unless d
      wt = (PokeAccess::Walkthrough.read_lines(d) rescue [])
      unless wt.empty?
        PokeAccess.speak(wt.join(". "), true)
        # :brief is the whole point of the split -- having said what to do, stop.
        return if mode == :brief
      end
      case kind(d)
      when :state  then state_read(d)
      when :facing then facing_read(d)
      when :push   then PokeAccess::Push.read(d)
      when :laser  then PokeAccess::Laser.read(d)
      when :walkthrough then nil
      else grid_read(d)
      end
    rescue StandardError
