module PokeAccess
  ...
  module Game
    ...
    class Definition
      ...
      # Registers a reader for events this game intercepts, whose own command list is therefore empty (see
      # Locator::CUSTOM_EVENTS). Yields the event; answer its spoken name, or nil to pass. The answer also
      # makes the event count as interactive, so it survives the hide-non-interactive filter and pings.
      def event_reader(&blk); PokeAccess::Locator.register_event_reader(&blk); end
