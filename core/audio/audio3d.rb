module PokeAccess
  ...
  module Audio3D
    ...
    CHANNEL_FILES = [
      ...
      [:mark, "pa3d_mark.wav", 0], [:roamer, "pa3d_roamer.wav", 0],
    ...
    def self.type_vol(t)
      key = (t == :hazard || t == :trap || t == :control || t == :push || t == :roamer) ? :audio3d_object : "audio3d_#{t}"
    ...
    def self.line_clear?(x0, y0, x1, y1)
      ...
      until x == x1 && y == y1
        ...
        return false unless (PokeAccess::Pathfinder.player_passable?(x, y, d) rescue true)
        x = nx; y = ny
      end
      true
    rescue StandardError
    ...
    # Fires one positional one-shot on a channel at a map tile, outside the paced emitter rotation: for a
    # cue the GAME decides the timing of (a roamer's flash) rather than one the sonar decides. Answers
    # false when the engine is not up, which is the caller's signal to fall back to a flat pre-panned file.
    #
    # It borrows the emitter path's occlusion raycast, so a cue from behind a wall is muffled exactly as a
    # ping from the same tile would be. drop_occluded is the one dial a caller gets over that: pass false
    # for a cue that mirrors something the GAME shows, since a top-down map draws its animations through
    # walls and dropping the cue would delete a signal the sighted player still receives. The three answers
    # are then distinct -- true played, :occluded deliberately silent (truthy, so callers do NOT fall back),
    # false the engine is unavailable and a flat file should be played instead.
    def self.play_at(sym, x, y, vol = nil, drop_occluded = true)
      return false unless @ready && $game_player
      return false if (nav_off? rescue false)
      ch = @ch[sym]
      return false unless ch && ch >= 0
      if drop_occluded && occlusion_mode == :hide && !line_clear?($game_player.x, $game_player.y, x, y)
        return :occluded
      end
      send_master
      set_occlusion(ch, [x, y])
      SET.call(ch, x * TILE_UNITS, y * TILE_UNITS, (vol || type_vol(sym)).to_i, 1)
      true
    rescue StandardError
      false
    end
    ...
    def self.ray(px, py, side)
      ...
      (1..wall_range).detect do |i|
        ...
        !(PokeAccess::Pathfinder.player_passable?(cx, cy, dir) rescue true)
      end
    end
