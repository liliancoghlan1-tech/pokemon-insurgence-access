module PokeAccess
  ...
  # True if this exact line was voiced a moment ago -- the same test say_dialogue dedups on, exposed so
  # the page reader can ask it too. A dedicated reader (battle, minigames, screen messages) speaks a line
  # BEFORE the engine routes it through the message window, and without this the window would then read
  # the same words a second time, page by page.
  def self.recently_said?(text, window = 0.5)
    return false if @last_say.nil? || @last_say_t.nil?
    now = (clock rescue 0)
    text == @last_say && (now - @last_say_t) < window
    ...
    class << Kernel
      unless method_defined?(:pbMessageDisplay__access_orig)
        ...
        def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil, &block)
          PokeAccess::MessagePages.begin_message(message)
          begin
            pbMessageDisplay__access_orig(msgwindow, message, letterbyletter, commandProc, &block)
          ensure
            PokeAccess::MessagePages.end_message
          end
        end
      end
    end
