...
module PokeAccess
  module MoveRelearnerGen6
    ...
    # True for the older relearner whose detail panel is drawn by pbRefreshInfo(move) and has no pbDrawMoveList.
    def self.info_layout?(scene)
      scene.respond_to?(:pbRefreshInfo, true) && !scene.respond_to?(:pbDrawMoveList, true)
    rescue StandardError
      false
    end

    # Set while the scene is opening, so the first move's detail queues behind the "teach which move" line
    # instead of cutting it off.
    def self.opening=(v); @opening = v; end

    # Speaks one move's full detail, or "Cancel" for the list's last row (the scene passes move 0 for it).
    def self.info_detail(id)
      s = (id.nil? || id.to_i == 0) ? PokeAccess::I18n.t(:pc_cancel) : PokeAccess::MoveInfo.by_id_via_data(id)
      PokeAccess.speak(s, !@opening) if s && !s.to_s.empty?
    rescue StandardError
      nil
    end
  end
...
PokeAccess::Hooks.after_hook(PokeAccess::MoveRelearnerGen6::SCENE, :pbStartScene, :hook_container => true) do |scene, _r, _a|
  ...
  PokeAccess.dedicate(PokeAccess.sprite(scene, "list")) if PokeAccess::MoveRelearnerGen6.info_layout?(scene)
  PokeAccess::MoveRelearnerGen6.opening = false
end
...
# The OLDER layout (Essentials v15, kept by Insurgence): no pbDrawMoveList. The list is a plain command
# window in sprite "list", and pbRefreshInfo(move) repaints a side panel with type, power, PP, accuracy and
# description whenever the cursor moves. Only the bare name reached the player. Read the panel's move instead.
PokeAccess::Hooks.before_hook(PokeAccess::MoveRelearnerGen6::SCENE, :pbStartScene) do |scene, args|
  if PokeAccess::MoveRelearnerGen6.info_layout?(scene)
    PokeAccess::MoveRelearnerGen6.opening = true
    pk = args[0]
    PokeAccess.speak(PokeAccess::I18n.t(:rel_teach_which, :name => (pk.name rescue "")), true) if pk
  end
end
PokeAccess::Hooks.after_hook(PokeAccess::MoveRelearnerGen6::SCENE, :pbRefreshInfo, :optional => true) do |scene, _r, args|
  PokeAccess::MoveRelearnerGen6.info_detail(args[0]) if PokeAccess::MoveRelearnerGen6.info_layout?(scene)
end
