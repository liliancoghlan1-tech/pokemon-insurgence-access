...
#
# Only when the appearance actually CHANGES (or while choosing one at a new game). Games also call it to
# re-apply the look the player already has -- Insurgence does on every Surf mount and dismount, through
# pbChangeBackToNormal -- and each of those read "Appearance 4, Girl" aloud, twice, over the route directions.
PokeAccess::Hooks.wrap_global("pbChangePlayer", "hook_pbChangePlayer_prev", :before) do |_args|
  PokeAccess::Appearance.instance_variable_set(:@prev_id, ($PokemonGlobal.playerID rescue nil))
end
PokeAccess::Hooks.wrap_global("pbChangePlayer", "hook_pbChangePlayer", :after) do |args, _r|
  prev = PokeAccess::Appearance.instance_variable_get(:@prev_id)
  if PokeAccess::Appearance.selecting? || prev.nil? || prev != ($PokemonGlobal.playerID rescue args[0])
    PokeAccess::Appearance.announce(args[0])
  end
end
