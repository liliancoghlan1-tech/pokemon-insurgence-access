module PokeAccess
  ...
  module DexEntry
    ...
    # The entry the game shows the moment a new species is caught.
    #
    # It is a DIFFERENT path from the browsable Pokedex: the battle scene calls pbShowPokedex, which opens
    # the dex scene straight at pbStartDexEntryScene ("Used only when capturing a new species", says the
    # engine's own comment) and never touches pbChangeToDexEntry, which is what the browsable reader hooks.
    # So the one entry a player most wants read to them was the one that never was.
    def self.capture_entry(species)
      return if species.nil?
      nm, kind, entry = (PokeAccess::Data.species_entry(species) rescue [nil, nil, nil])
      nm = (PokeAccess::Data.species_name(species) rescue nil) if nm.nil? || nm.to_s.empty?
      parts = []
      parts.push(nm.to_s) if nm && !nm.to_s.empty?
      parts.push(PokeAccess::I18n.t(:dex_category, :cat => kind.to_s)) if kind && !kind.to_s.empty?
      parts.push(PokeAccess.clean(entry).to_s) if entry && !entry.to_s.strip.empty?
      return if parts.empty?
      PokeAccess.speak(PokeAccess::Util.join_parts(parts), false)
    rescue StandardError
      nil
    end
...
PokeAccess::Hooks.after_hook("PokemonPokedexScene", :pbStartDexEntryScene, :optional => true) { |_s, _r, a| PokeAccess::DexEntry.capture_entry(a[0]) }
