module PokeAccess
  ...
  module Menus
    ...
    # The shop list, with what each item DOES.
    #
    # The screen prints the description in a window of its own, updated inside the scene's own input loop,
    # so there is no method to hook and a shelf of interesting things read as a list of names and prices --
    # fine if you already know what a Shell Bell is. Asked of the scene's ADAPTER first, because a mart can
    # sell items, TMs or another currency entirely and the adapter is the thing that knows which; the data
    # layer covers a game whose adapter is shaped differently. A whitespace answer counts as no answer.
    def_extractor("Window_PokemonMart") do |win, i|
      ...
      line = price ? "#{ad.getDisplayName(stock[i])}, #{price}" : ad.getDisplayName(stock[i]).to_s
      d = (ad.getDescription(stock[i]) rescue nil)
      d = (PokeAccess::Data.item_description(stock[i]) rescue nil) if d.nil? || d.to_s.strip.empty?
      next line if d.nil? || d.to_s.strip.empty?
      "#{line}. #{PokeAccess.clean(d)}"
    end
