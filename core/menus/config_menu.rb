  ...
  module ConfigMenu
    ...
    def self.items
      ...
      when :audio
        ...
        rows
      when :assist
        # A game with no universal-Exp-Share switch of its own is not offered the setting: a row that
        # cannot do anything is worse than a missing one when the menu is read aloud.
        rows = PokeAccess::Config.schema_group(:assist).reject do |r|
          r[0] == :assist_expshare && !(PokeAccess::Assist.expshare_available? rescue false)
        end.map { |r| { :kind => :setting, :row => r } }
        rows.push({ :kind => :back, :label => :back })
        rows
      else
    ...
    def self.value_text(row)
      ...
      when :encmode then t("asenc_#{v}".to_sym)
      when :expmode then t("asexp_#{v}".to_sym)
      else v.to_s
      end
    end
    ...
    def self.adjust_setting(row, dir)
      ...
      when :encmode
        cycle(row, key, dir, PokeAccess::Assist::ENCOUNTER_MODES, "asenc_")
      when :expmode
        cycle(row, key, dir, PokeAccess::Assist::EXP_MODES, "asexp_")
      end
    end
