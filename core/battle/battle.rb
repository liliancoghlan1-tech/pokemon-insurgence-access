module PokeAccess
  ...
  module Battle
    ...
    def self.battle_started; @in_battle = true; @mega_told = false; end

    # True the FIRST time this battle that the fight menu offers Mega Evolution (or whatever mechanic sits on
    # that button), so it is announced once rather than every turn. The button being lit is the only sign a
    # sighted player gets, and a silent one is indistinguishable from not having it: she had the stone in her
    # bag rather than held and nothing said so.
    def self.mega_ready!
      return false if @mega_told
      @mega_told = true
      true
    end
    ...
    def self.battler_state(b, hide_exact = false)
      ...
      # Gender belongs with the name: it decides Attract, Rivalry and Cute Charm, and on screen it is a
      # symbol beside the name that costs a sighted player nothing to read.
      g = (PokeAccess::Party.gender_word(b.pokemon) rescue nil)
      t += ", " + g if g
    ...
    def self.foe_info
      ...
      bs.each_with_index do |b, i|
        ...
        g = (PokeAccess::Party.gender_word(pk) rescue nil)
        line += ", " + g if g
    ...
    # Delegates to DayNight, which copes with the old engines' required time argument.
    def self.time_of_day
      PokeAccess::DayNight.band_key
    rescue StandardError
      nil
    end
    ...
    def self.announce_overworld
      ...
      # The clock, the band, and when it next turns night or day -- an evolution or encounter that needs
      # night is otherwise a guess at the screen's tint.
      tod = (PokeAccess::DayNight.status_text rescue nil)
      tod ||= (k = time_of_day) ? PokeAccess::I18n.t(k) : nil
      parts.push(tod) if tod
      fe = field_event_text
      parts.push(fe) if fe
