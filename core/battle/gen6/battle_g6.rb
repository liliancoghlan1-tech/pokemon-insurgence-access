...
[["megaButton=", :@access_mega, :bt_mega_on, :bt_mega_off],
 ...
 ["mode=", :@access_mega, :bt_mega_on, :bt_mega_off]].each do |setter, slot, on, off|
  ...
  PokeAccess::Hooks.after_hook("FightMenuDisplay", setter.to_sym) do |disp, _r, args|
    ...
    # Offered, and not yet mentioned this battle: say so once. Without it the button is silent until she
    # presses the key, which is no help to someone who does not know it is lit.
    if k.nil? && v == 1 && slot == :@access_mega && PokeAccess::Battle.mega_ready!
      PokeAccess.speak(PokeAccess::I18n.t(:bt_mega_ready), false)
    end
  end
end
