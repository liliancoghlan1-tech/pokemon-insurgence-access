module PokeAccess
  ...
  module Party
    ...
    def self.announce_party(scene, party, idx, oldidx)
      ...
      ann = party_annotation(scene, idx) if party_slot?(party, idx)
      t = "#{t}, #{ann}" if ann
      PokeAccess.speak(t, true)
    end

    # The word a chooser prints on a slot -- ABLE / NOT ABLE when picking a Pokemon for the Move Relearner or a
    # TM, ENTERED / BANNED on a challenge entry -- or nil. It is the whole point of those screens and was only
    # ever drawn. Screens print it in capitals; lowered so a screen reader says the word, not the letters.
    def self.party_annotation(scene, idx)
      spr = (PokeAccess.ivar(scene, :@sprites)["pokemon#{idx}"] rescue nil)
      return nil unless spr && spr.respond_to?(:text)
      s = PokeAccess.clean(spr.text.to_s).to_s.strip
      return nil if s.empty?
      s == s.upcase ? s.downcase : s
