module PokeAccess
  ...
  module SummaryGen6
    ...
    def self.stats_text(pk)
      ...
      if ab && !ab.to_s.empty?
        t += " " + PokeAccess::I18n.t(:sum_ability, :a => ab)
        # The name of an ability is not the ability. On screen its effect is printed underneath; spoken,
        # it was the one thing the stats page left out.
        d = (PokeAccess::Data.ability_description(pk.ability) rescue nil)
        t += ". " + PokeAccess.clean(d).to_s if d && !d.to_s.strip.empty?
      end
      t
    rescue StandardError
      nil
